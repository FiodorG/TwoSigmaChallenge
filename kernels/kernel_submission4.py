from libraries import utilities, kagglegym
import numpy as np
import pandas as pd
import random
import scipy
...
#########################################################
def add_y_inferred(df, alpha = 0.9327):
...
train = train[y_is_within_cut]

rnd=17

add_nas_ft=True
nas_cols=['technical_9', 'technical_0', 'technical_32', 'technical_16', 'technical_38', 'technical_44', 'technical_20', 'technical_30', 'technical_13']
add_diff_ft=True
...
class recurrent_linear_approx():
    ...
    def fit(self, train, y):
        internal_model=linear_model.RidgeCV(fit_intercept=True, cv=model_selection.TimeSeriesSplit(n_splits=2))
        bestscore=1e15
        better=True
...
cols2fit=['technical_22','technical_20', 'technical_30_d', 'technical_20_d', 'technical_30', 'technical_13', 'technical_34']
cols2removezeros = ['technical_20', 'technical_30_d', 'technical_20_d', 'technical_30', 'technical_13', 'technical_13_d']
models=[]
columns=[]
residuals=[]
for elt in cols2fit:
    print("fitting linear model on ", elt)
    model=recurrent_linear_approx(quant=.99, limit_size_train=.85)
    
    if elt in cols2removezeros:
        train_temp = train[train[elt] != 0]
    else:
        train_temp = train
    
    model.fit(train_temp.loc[:,[elt]],train_temp.loc[:, 'y'])
    models.append(model)
    columns.append([elt])
    residuals.append(abs(model.predict(train_temp[[elt]].fillna(d_mean))-train_temp.y))
