...
for col in all_columns:
    ...
    model = linear_model.Lars(n_nonzero_coefs=1, fit_intercept=True, normalize=True)
    #model = linear_model.ElasticNetCV()
    #model = linear_model.BayesianRidge()

    x_train = np.diff(df_train[columns], axis=0)
    y_train = np.diff(df_train[['y']], axis=0)

    model.fit(x_train, y_train)
    columns_selected = [columns[col_id] for col_id in model.active_]

    x_test = np.diff(df_test[columns].fillna(0), axis=0)
    y_test = model.predict(x_test)
    #y_fit = get_weighted_y(y_test, df_train, df_test)

    score = r_score(np.diff(y, axis=0), y_test)
    scores.append(score)
